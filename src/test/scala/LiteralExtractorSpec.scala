...
import fixedpoint.{FixedPoint, fromIntToBinaryPoint, fromDoubleToLiteral, fromBigDecimalToLiteral, NumBP}
...
class LiteralExtractorSpec extends ChiselFlatSpec {
  ...
  "literals declared outside a builder context" should "compare with those inside builder context" in {
    ...
    class LitInsideOutsideTester(outsideLiteral: InsideBundle) extends BasicTester {
      val insideLiteral = (new InsideBundle).Lit(_.x -> 7.S, _.y -> 6.125.F(8.W, 4.BP))
    ...
    val outsideLiteral = (new InsideBundle).Lit(_.x -> 7.S, _.y -> 6.125.F(8.W, 4.BP))
